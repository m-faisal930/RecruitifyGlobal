...
//   const ApplicationDetails = () => {
//   const { id } = useParams(); // get the applicant ID from URL
...
//         setApplication(res.data.data); // assuming your controller sends data like { success: true, data: applicant }
//       } catch (err) {
...
const ApplicationDetails = () => {
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      ...
      await axios.put(`http://localhost:5000/api/applicants/status/${id}`, {
        status: newStatus,
      });
    } catch (err) {
  ...
  const saveNotes = async () => {
    try {
      await axios.put(`http://localhost:5000/api/applicants/${id}`, {
        notes: notes,
      });
      setIsEditingNotes(false);
  ...
  const downloadFile = async (fileType) => {
    try {
      window.open(
        `http://localhost:5000/api/applicants/${id}/download/${fileType}`,
        '_blank'
      );
    } catch (err) {
